...
def normalizeColumn(column):
    ...
    #asdres = asd for x in column
    #result = [(x - median) / asdres]
    print 'asd:', asd, 'column:', column
    result = column
    try:
        if (asd != 0.0):
            result = [(x - median) / asd for x in column]
    except:
        print 'normalizeColumn exception: ', sys.exc_info()[0]
    print 'result', result
    return result
...
class hClusterer:
    ...
    def __init__(self, filename):
        ...
        for i in range(rows):
            ...
            print 'min distance', minDistance, 'nearestPair', nearestPair, 'neighbours', neighbors
